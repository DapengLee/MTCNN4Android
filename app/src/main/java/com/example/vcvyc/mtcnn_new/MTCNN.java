...
public class MTCNN {
    ...
    MTCNN(AssetManager mgr) {
        try {
            final String MODEL_FILE = "file:///android_asset/mtcnn_freezed_model.pb";
            inferenceInterface = new TensorFlowInferenceInterface(mgr, MODEL_FILE);
    ...
    private void pNetForward(Bitmap bitmap, float[][] PNetOutProb, float[][][] PNetOutBias) {
        ...
        inferenceInterface.feed(P_NET_IN_NAME, pNetIn, 1, w, h, 3);
        inferenceInterface.run(P_NET_OUT_NAME, false);
        ...
        inferenceInterface.fetch(P_NET_OUT_NAME[0], pNetOutP);
        inferenceInterface.fetch(P_NET_OUT_NAME[1], PNetOutB);
        //【写法一】先翻转，后转为2/3维数组
    ...
    private Vector<Box> generateBoxes(float[][] prob, float[][][] bias, float scale, float threshold) {
    ...
    private void boundingBoxRegression(Vector<Box> boxes) {
        for (int i = 0; i < boxes.size(); i++)
    ...
    private Vector<Box> pNet(Bitmap bitmap, int minSize) {
        ...
        while (currentFaceSize <= whMin) {
            ...
            //(3)数据解析
            Vector<Box> curBoxes = generateBoxes(pNetOutProb, PNetOutBias, scale, PNetThreshold);
    ...
    private void cropAndResize(Bitmap bitmap, Box box, int size, float[] data) {
        //(2)crop and resize
        Matrix matrix = new Matrix();
    ...
    private void RNetForward(float[] RNetIn, Vector<Box> boxes) {
        ...
        //feed & run
        inferenceInterface.feed(R_NET_IN_NAME, RNetIn, num, 24, 24, 3);
        inferenceInterface.run(R_NET_OUT_NAME, false);
        //fetch
        float[] RNetP = new float[num * 2];
        float[] RNetB = new float[num * 4];
        inferenceInterface.fetch(R_NET_OUT_NAME[0], RNetP);
        inferenceInterface.fetch(R_NET_OUT_NAME[1], RNetB);
        //转换
        for (int i = 0; i < num; i++) {
    ...
    private void oNetForward(float[] ONetIn, Vector<Box> boxes) {
        ...
        //feed & run
        inferenceInterface.feed(O_NET_IN_NAME, ONetIn, num, 48, 48, 3);
        inferenceInterface.run(O_NET_OUT_NAME, false);
        //fetch
        float[] ONetP = new float[num * 2]; //prob
        float[] ONetB = new float[num * 4]; //bias
        float[] ONetL = new float[num * 10]; //landmark
        inferenceInterface.fetch(O_NET_OUT_NAME[0], ONetP);
        inferenceInterface.fetch(O_NET_OUT_NAME[1], ONetB);
        inferenceInterface.fetch(O_NET_OUT_NAME[2], ONetL);
        //转换
        for (int i = 0; i < num; i++) {
            //prob
    ...
    private Vector<Box> oNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        int ONetInIdx = 0;
        for (int i = 0; i < num; i++) {
            cropAndResize(bitmap, boxes.get(i), 48, curCrop);
    ...
    public Vector<Box> detectFaces(Bitmap bitmap, int minFaceSize) {
        ...
        squareLimit(boxes, bitmap.getWidth(), bitmap.getHeight());
        //【2】RNet
